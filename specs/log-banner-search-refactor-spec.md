...
1. **日志文件化**：运行 `zcode` 时控制台不显示日志，日志保存到本地文件
...
<configuration>
    <!-- 定义日志目录 -->
    <property name="LOG_DIR" value="${user.home}/.zcode/logs" />

    <appender name="FILE" class="ch.qos.logback.core.rolling.RollingFileAppender">
        <file>${LOG_DIR}/zcode.log</file>
        <rollingPolicy class="ch.qos.logback.core.rolling.SizeAndTimeBasedRollingPolicy">
            <fileNamePattern>${LOG_DIR}/zcode.%d{yyyy-MM-dd}.%i.log</fileNamePattern>
...
1. 运行 `zcode` 后控制台只显示 banner + prompt，无任何日志行
2. `~/.zcode/logs/zcode.log` 文件中能看到完整日志（启动信息、工具调用、错误堆栈）
...
- 修改 `ZCodeMain.java`：调用处传入 config
