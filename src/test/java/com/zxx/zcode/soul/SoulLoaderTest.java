...
class SoulLoaderTest {
    ...
    void workDirOverridesUserHomeSoulsDir() throws Exception {
        Path root = Files.createTempDirectory("zcode-soul-test");
        Path fakeHome = root.resolve("home");
        Path work = root.resolve("work");
        Files.createDirectories(fakeHome.resolve(".zcode/souls"));
        Files.createDirectories(work.resolve(".zcode/souls"));
        Files.writeString(fakeHome.resolve(".zcode/souls/demo.json"), """
                {"id":"demo","display_name":"FromHome","persona":"home persona","peers":[]}
                """);
        Files.writeString(work.resolve(".zcode/souls/demo.json"), """
    ...
    void fallsBackToUserHomeWhenWorkDirHasNoSoulFile() throws Exception {
        Path root = Files.createTempDirectory("zcode-soul-test2");
        Path fakeHome = root.resolve("h");
        Path work = root.resolve("w");
        Files.createDirectories(fakeHome.resolve(".zcode/souls"));
        Files.createDirectories(work.resolve(".zcode/souls"));
        Files.writeString(fakeHome.resolve(".zcode/souls/onlyhome.json"), """
    ...
    void nestedSoulJsonPreferredOverFlatInWorkDir() throws Exception {
        Path root = Files.createTempDirectory("zcode-soul-nested");
        Path work = root.resolve("work");
        Files.createDirectories(work.resolve(".zcode/souls/demo"));
        Files.writeString(work.resolve(".zcode/souls/demo/soul.json"), """
