...
import com.zxx.zclaw.llm.model.ContentBlock;
import com.zxx.zclaw.llm.model.Message;
import com.zxx.zclaw.llm.model.ToolCall;
...
public final class ConversationContextEstimator {
    ...
    /**
     * Optional reference window for /status (e.g. 200000). Set {@code ZCLAW_CONTEXT_WINDOW} to a positive int.
     */
    public static int referenceContextWindowFromEnv() {
        String v = System.getenv("ZCLAW_CONTEXT_WINDOW");
        if (v == null || v.isBlank()) {
            return 0;
        }
        try {
