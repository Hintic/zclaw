...
import com.zxx.zcode.llm.model.WebSearchResult;
import com.zxx.zcode.tool.ToolRegistry;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import com.zxx.zcode.cli.CliIndent;
import com.zxx.zcode.cli.MarkdownRenderer;
import com.zxx.zcode.cli.StreamThinkingStripper;
import com.zxx.zcode.cli.ThinkingIndicator;
import com.zxx.zcode.cli.ToolResultFormatter;
import com.zxx.zcode.memory.MemorySummarizer;
import com.zxx.zcode.memory.ProjectMemory;
...
public class AgentLoop {
    ...
    private String buildSystemPrompt(Path workDir) {
        ...
        if (soul.isDefault() && soul.getPersona().isBlank()) {
            sb.append("You are z-code, a coding assistant that helps users with software engineering tasks.\n");
        } else {
            sb.append("You are ").append(soul.getDisplayName());
            sb.append(", a z-code assistant instance with soul id `").append(soul.getId()).append("`.\n");
            if (!soul.getPersona().isBlank()) {
                sb.append("\n## Persona\n");
                sb.append(soul.getPersona().trim()).append("\n\n");
            }
            sb.append("You follow the same engineering standards as z-code for this workspace.\n");
        }
        ...
        sb.append("- soul_mail: Message another z-code process in this directory with a different --soul; ");
        ...
        sb.append("The user may run a second z-code in the same directory with another --soul. ");
     ...
     * Optional {@code ZCODE_CONTEXT_WINDOW} adds a %-of-window hint in {@link ConversationContextSnapshot#referencePercentSuffix()}.
     */
